...
from Models.ControllerModel import make_controller, simulate
...
def initialize_settings(sigma_init=0.1, sigma_decay=0.9999):
  ...
  filedir = 'C:/Users/benat/OneDrive/Dokumentuak/World Models CandNo215816/resultsCarRacing/'
  if not os.path.exists(filedir):
      os.makedirs(filedir)

  filebase = filedir+'CarRacing-v0'+'.'+optimizer+'.'+str(num_episode)+'.'+str(population)
  print("\n",filebase,"\n")
...
def master():
  for run_i in range(5):
    initialize_settings(sigma_init=0.1, sigma_decay=0.9999)
    global test_env
    test_env = make_env(gym_env="CarRacing-v0", dream_env=False, full_episode = False, load_model=True) #True
    ...
    filename_bestPerf = "C:/Users/benat/OneDrive/Dokumentuak/World Models CandNo215816/ResultsGraph"+"/"+"Frozen2bestCarNormal20"+".npy"
    filename_avg = "C:/Users/benat/OneDrive/Dokumentuak/World Models CandNo215816/ResultsGraph"+"/"+"Frozen2avgCarNormal20"+".npy"
    filename_worse = "C:/Users/benat/OneDrive/Dokumentuak/World Models CandNo215816/ResultsGraph"+"/"+"Frozen2worstCarNormal20"+".npy"
...
def slave(packet_list):
  global env, test_env
  result_packet = []
  env = make_env(gym_env="CarRacing-v0", dream_env=False,full_episode = False, load_model=True) #True
  for i in range(1, num_worker):
    ...
    for solution in solutions:
      ...
      jobidx = int(jobidx)
      seed = int(seed)
      fitness, timesteps = worker(weights)
...
def worker(weights):
  ...
  reward_list, t_list = simulate(controller, env,
        render_mode=True, num_episode=2, seed=0)
...
def main():
  ...
  exp_name = 'CarRacing-v0'
  batch_mode = 'mean'

  initialize_settings(0.1, 0.999) 
  master()
