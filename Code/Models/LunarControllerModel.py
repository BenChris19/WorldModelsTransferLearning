...
class Controller:
  ''' simple one layer model for lunar lander '''
  def __init__(self):
  ...
  def get_action(self, h):
    action = np.tanh(np.dot(h, self.weight) + self.bias) #np.tanh(
    return action
...
def simulateLunar(controller, env,  render_mode=True, num_episode=5, seed=-1):
  reward_list = []
  t_list = []
