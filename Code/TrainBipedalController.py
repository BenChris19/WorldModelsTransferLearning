...
from Models.ControllerModelBipedal import make_controller, simulateBipedal
...
def initialize_settings(sigma_init=0.1):
  ...
  filedir = 'C:/Users/benat/OneDrive/Dokumentuak/World Models CandNo215816/resultsBipedal/'
  if not os.path.exists(filedir):
      os.makedirs(filedir)

  filebase = filedir+"BipedalWalker-v2"+'.'+optimizer+'.'+str(num_episode)+'.'+str(population)
  print("\n",filebase,"\n")
...
def master():
  for run_i in range(5):
    global test_env
    test_env = make_env(gym_env= "BipedalWalker-v2", dream_env=False,full_episode = False, load_model=True)
    ...
    filename_bestPerf = "C:/Users/benat/OneDrive/Dokumentuak/World Models CandNo215816/ResultsGraph"+"/"+"worstTransferbestBipedalWalker"+".npy"
    filename_avg = "C:/Users/benat/OneDrive/Dokumentuak/World Models CandNo215816/ResultsGraph"+"/"+"worstTransferavgBipedalWalker"+".npy"
    filename_worse = "C:/Users/benat/OneDrive/Dokumentuak/World Models CandNo215816/ResultsGraph"+"/"+"worstTransferBipedalWalker"+".npy"
...
def slave(packet_list):
  global env, test_env
  result_packet = []
  env = make_env(gym_env="BipedalWalker-v2", dream_env=False,full_episode = False, load_model=True) 
  for i in range(1, num_worker):
...
def worker(weights,seed):
  ...
  reward_list, t_list = simulateBipedal(controller, env,
...
def main():
  ...
  cap_time_mode= False
  seed_start = 0
  env_name = 'Bipedal Walker'
  exp_name = 'BipedalWalker-v2'
  batch_mode = 'mean'
